...
public class CheckersModel {
	 ...
	 */
	public final void movePiece(final int startRow, final int startCol, final int endRow, final int endCol) {
	 ...
	 */
	public final boolean[] canMove(final int startRow, final int startCol, final int endRow, final int endCol) {
		...
		if (this.piece.getPlayer() == Player.Red || (this.piece.getPlayer() == Player.Black && this.piece.isKing())) {
		...
		if (this.piece.getPlayer() == Player.Black || (this.piece.getPlayer() == Player.Red && this.piece.isKing())) {
			if (startRow - 1 == endRow && (startCol + 1 == endCol || startCol - 1 == endCol)) {
				type[0] = true;
				return type;
			} else if (startRow - 2 == endRow) {
	 ...
	 */
	public final int[] jumped(final int startRow, final int startCol, final int endRow, final int endCol) {
			...
			for (int col = 0; col < 8; col++) {
				...
				if (this.piece.getPlayer() == p) {
					...
					for (int dir = 0; dir < 4; dir++) {
						loc = this.checkForMoreJumps(p, this.piece.isKing(), dir, loc[0], loc[1], pRow, pCol);
						while (loc[0] != -1) {
							...
							if (dir == 3) {
								...
								if (jumpList.size() > 0) {
									...
									if (jumps == 0) {
										...
										pRow = jumpList.get(listCounter).get(jumpList.get(listCounter).size() - 6);
										pCol = jumpList.get(listCounter).get(jumpList.get(listCounter).size() - 5);
										loc[0] = jumpList.get(listCounter).get(jumpList.get(listCounter).size() - 4);
										loc[1] = jumpList.get(listCounter).get(jumpList.get(listCounter).size() - 3);
										tempRow = loc[0];
										tempCol = loc[1];
									}
									listCounter++;
